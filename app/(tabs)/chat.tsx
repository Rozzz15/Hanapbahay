    ...
    useEffect(() => {
        const loadConvos = async () => {
            try {
                ...
                const myConvos = user ? convos.filter(c => c.participantIds.includes(user.id)) : [];
                setConversations(myConvos);
                
                const mapped: ChatItem[] = myConvos.map(c => {
                    const otherUserId = user ? c.participantIds.find(pid => pid !== user.id) : undefined;
                    return {
                        id: c.id,
                        conversationId: c.id,
                        otherUserId: otherUserId,
                        name: `Owner ${otherUserId?.slice(-4) || 'Unknown'}`,
        ...
        <SafeAreaView className="flex-1 bg-white">
            ...
            <View className="bg-white px-6 py-4 border-b border-gray-200">
                <View>
                    ...
                    <Text className="text-gray-600">Chat with property owners</Text>
                </View>
            </View>
