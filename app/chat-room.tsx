    ...
    const loadMessages = useCallback(async () => {
        ...
        try {
            ...
            const uiMessages: Message[] = conversationMessages.map((msg: any) => ({
                ...
                isOwner: msg.senderId === user.id || msg.sender_id === user.id
            }));
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            try {
                ...
                if (conversation) {
                    const isOwner = (conversation as any).ownerId === user.id;
                    await db.upsert('conversations', conversationId as string, {
                        ...conversation,
                        lastMessageText: newMessage.trim(),
                        lastMessageAt: now,
                        unreadByOwner: isOwner ? (conversation as any).unreadByOwner || 0 : ((conversation as any).unreadByOwner || 0) + 1,
                        unreadByTenant: !isOwner ? (conversation as any).unreadByTenant || 0 : ((conversation as any).unreadByTenant || 0) + 1,
                        updatedAt: now
                    });
                }
            } catch (convError) {
