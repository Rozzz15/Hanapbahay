...
const ListingCard: React.FC<ListingType> = ({ image, title, location, rating, reviews, rooms, size, price }) => {
    ...
    const isMobile = width < 768; // Mobile threshold

    return (
        <Box className="flex-row bg-white rounded-2xl border border-gray-300 shadow-md overflow-hidden w-full max-w-sm">
            {/* Left Side - Image */}
            <Image 
                source={{ uri: image }} 
            ...
            {/* Right Side - Details */}
            <VStack className="flex-1 p-6 space-y-2">
                {/* Rating */}
                ...
                {/* Features */}
                <HStack className="items-center space-x-4 gap-1">
                    <HStack className="flex-row items-center space-x-1 gap-1">
